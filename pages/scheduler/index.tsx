...
const Scheduler: NextPage = () => {
    ...
    useEffect(() => {
        const snapshotCallback = scheduler.fetchSchedules((docs: Array<QueryDocumentSnapshot>) => {
            setSchedules(docs);
            setLoading(false);
        })
    ...
    let data = schedules.map(schedule => {
        ...
        return <Schedule key={scheduleData.id} data={scheduleData} reference={schedule.ref}></Schedule>
    })
